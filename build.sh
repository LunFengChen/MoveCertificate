...
if [ -n "$CERT_DIR" ] && [ -d "$CERT_DIR" ]; then
    # 如果 CERT_DIR 不是 certificates 目录本身，才需要复制
    CERT_DIR_REAL=$(cd "$CERT_DIR" && pwd)
    CERT_LOCAL=$(pwd)/certificates
    
    if [ "$CERT_DIR_REAL" != "$CERT_LOCAL" ]; then
        ...
        echo "Using existing certificates directory"
    fi
